...
const InputState& InputManager::get_input_state() 
{
...
{
	const bool* key_state = SDL_GetKeyboardState(NULL);  //<- (should be caching this I think)
	...
	{
		InputState input{
			.move_intent = glm::vec2(moveX, moveY),
			.is_sprinting = is_sprinting,
			.mouseX = mouse_x,
			.mouseY = mouse_y,
			.player_did_shoot = did_shoot
		};
		return input;
	}
